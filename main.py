...
def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            clear_screen()
            userLanguage = input(translate("\nElije tu nuevo idioma para continuar (es/en/fr/de): ", to_language=userLanguage))
            while userLanguage not in LANGUAGES:
                print(f"{RED}Idioma no válido. Por favor, ingrese un idioma válido.{RESET}")
                userLanguage = input(f"Elije tu nuevo idioma para continuar (es/en/fr/de): ")
            
        elif choice == "4":
            clear_screen()
            sleep(1)
            print(translate("Gracias por usar Instant Translator. ¡Hasta luego!", to_language=userLanguage))
            sleep(1)
            break
        
        else:
            print(translate("Opción no válida. Por favor, elija una opción válida.", to_language=userLanguage))
